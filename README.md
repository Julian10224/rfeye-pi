# RF Eye 0.9.7 for Raspberry Pi

This repository contains the complete **RF Eye 0.9.7 reference appliance** for the MHS35/CUQI-style 3.5-inch SPI touchscreen.
...
The installer reproduces the working 0.9.7 appliance path:
...
### How the band is searched (0.9.7)
...
frequency order, which is uncorrelated with signal strength; since 0.9.7 the
...
tight. Since 0.9.7 the debug page carries the best channel of the current band
...
RF Eye 0.9.7 uses a **TMB12A03 active buzzer**:
...
Since 0.9.7 a genuine black panel should no longer be possible from a software
fault: any exception raised while drawing a frame is caught, shown on the panel
as **RF EYE FAULT** with the exception text, the page and the release, and
appended once per distinct fault to `~/.local/state/rfeye/crash.log`. Before
that, one exception ended the process, systemd restarted it half a second later
(`Restart=always`), and a fault that repeated every frame became a restart loop
whose only visible symptom was a black screen. If a unit goes dark and shows no
fault screen, the cause is below the application: the compositor, the panel, or
the layout problem described next.
...
Since 0.9.7 the runtime falls back to the `display_profile` already recorded in
...
`VERSION` and `rfeye/config.py` identify this release as **0.9.7**.
