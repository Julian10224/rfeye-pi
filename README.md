...
0.9.18 fixed the refill gate and the field unit still logged no band pass at
all. The gate was only half of it.

A round is **one dwell** -- one capture, one ~100 kHz window, chosen by
`plan_dwell` from the head of the priority list. Locked carriers are rate
limited by `_site_verify_at`; candidates were queued unconditionally on every
single round. So a candidate anchors every dwell to its own neighbourhood. The
queued channels within +/-100 kHz of it come along for free, drain, and run
out -- and after that the pass never reaches the rest of the band again.

That is why the unit covered `ch=213` in one pass at 20:28 and then nothing for
an hour: two candidates 575 kHz apart, each anchoring alternate captures to its
own corner, the queue frozen at whatever was left.

Since 0.9.19 candidates alternate with the queue instead of pre-empting it, and
rotate among themselves so two that cannot share a dwell take turns. A
candidate is still re-tested every couple of rounds, which is far quicker than
it needs to be to reach three hits. With nothing queued they get every round,
because then there is nothing to starve.

Scenario 15 measures the thing that actually broke: how many channels *more
than 150 kHz from any candidate* the pass reaches in 40 rounds. Before the fix
that is 0 of 174; after it, 68.
...
network re-sweeps the whole band every `site_rescan_s`.

That interval was 300 s, chosen when a band pass itself took about 300 s. With
the 2.016 MS/s dwell of 0.9.26 a pass is about nine dwells, roughly 30 s, so
the five-minute wait was pure delay: measured on rfeye, after the first lock a
second carrier turned up only on the pass after next. In a car that is
kilometres of a site's uplinks going unwatched. Since 0.9.28 the default is
120 s. A pass then runs on alternate cycles for about a quarter of the time,
and each uplink channel is still examined about every 3 s while it does.

If a unit reports one locked carrier where the site plainly has more, read
`site_locked_count` on the debug page after two or three minutes rather than
immediately -- and remember that a carrier near the 8 dB floor locks and
unlocks with the weather.
...
Detection rate over 16-24 random positions of the burst within the dwell, at
16-30 dB -- see *One position is not a measurement* below for why it is
reported this way and not as a single verdict:

| uplink transmission | real duty | 0.9.32 | 0.9.33 | 0.9.34 |
| --- | --- | --- | --- | --- |
| 7 ms subslot (random access) | 1.4% | 0% | 62-81% | **100%** |
| 14 ms, one control burst | 2.7% | 0% | 93-100% | **100%** |
| 28 ms, two bursts | 5.4% | 100% | 100% | 100% |
| held call | 25% | 100% | 100% | 100% |
...
### One position is not a measurement (0.9.34)

0.9.33 passed every case above and was still wrong about the shortest burst.
Its suite placed the burst at one fixed point in the dwell; swept across the
dwell instead, a 7 ms subslot was accepted on **68% of positions at 30 dB** --
and on 62% at 22 dB, which is worse at a *stronger* signal. A result that does
not improve with SNR is not a threshold, it is variance.

The cause is the occupied-bandwidth walk. It steps outward from the channel
centre and stops at the first bin below -10 dB, so it is only as stable as the
single noisiest bin it passes. At 281 Hz resolution a 25 kHz carrier is ~89
bins -- far more frequency resolution than a width measurement needs -- and
under the peak hold each bin of a 7 ms burst rests on one or two FFT rows, so
the raw profile carries several dB of scatter. The walk terminated wherever
the scatter happened to dip.

Since 0.9.34 the walk runs on a spectrum smoothed to ~1.4 kHz first, which
spends that surplus resolution on variance instead. Only this measurement is
smoothed. The level, the channel-edge valley and the flatness are calibrated
ratios and are left on the raw spectrum: smoothing those moves the noise
reference by several dB, and measured over the same sweep it cost a 14 ms
burst at 12 dB most of its margin. A continuous downlink carrier measures
21.4-21.7 kHz after the change against 21.4 kHz before, so the 14-30 kHz
acceptance window still means what it was calibrated to mean.

`scripts/tetra-phy-selftest.py` now sweeps eight burst positions per case at
25 and 18 dB rather than trusting one. Against 0.9.33 that sweep fails on
three of eight subslot positions; against this release it passes on all of
them.

**The floor, stated rather than assumed.** At 12 dB a *single* burst is not
reliably separable from noise by the occupied-bandwidth measurement, in any
release -- the -10 dB point runs out into the noise and the walk reaches its
cap. Two slots at 12 dB are solid, and the suite asserts exactly that. Next to
a vehicle this does not arise: a handset a few metres away arrives at 25-40 dB.
It matters for a terminal far enough away that only one short burst is on air.
