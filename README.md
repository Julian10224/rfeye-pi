# RF Eye 0.9.6 for Raspberry Pi

This repository contains the complete **RF Eye 0.9.6 reference appliance** for the MHS35/CUQI-style 3.5-inch SPI touchscreen.
...
The installer reproduces the working 0.9.6 appliance path:
...
### How the band is searched (0.9.6)
...
frequency order, which is uncorrelated with signal strength; since 0.9.6 the
...
tight. Since 0.9.6 the debug page carries the best channel of the current band
...
RF Eye 0.9.6 uses a **TMB12A03 active buzzer**:
...
Since 0.9.6 the runtime falls back to the `display_profile` already recorded in
...
`VERSION` and `rfeye/config.py` identify this release as **0.9.6**.
