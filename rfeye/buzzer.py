...
class GPIOBuzzer:
    """
    Raspberry Pi GPIO buzzer driver.

    passive=True:
      Uses software PWM so different tones can be generated.

    passive=False:
      Drives an active buzzer HIGH/LOW. The buzzer itself generates the tone.
    """
    ...
    def _beep_worker(self, token, frequency, duration_ms, duty):
        try:
            with self.lock:
                ...
                if self.passive:
                    frequency = max(50, frequency)
                    self.pwm.ChangeFrequency(frequency)
                    self.pwm.ChangeDutyCycle(max(0, min(100, duty)))
                else:
                    GPIO.output(
                        self.pin,
                        GPIO.HIGH if self.active_high else GPIO.LOW
                    )
            ...
            with self.lock:
                ...
                if self.passive:
                    ...
                    GPIO.output(
                        self.pin,
                        GPIO.LOW if self.active_high else GPIO.HIGH
                    )
        except Exception:
            pass
